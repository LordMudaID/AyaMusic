...
from AyaMusic import YouTube, app
from AyaMusic.core.call import Aya
from AyaMusic.misc import SUDOERS, db
from AyaMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from AyaMusic.utils.decorators.language import languageCB
from AyaMusic.utils.formatters import seconds_to_min
from AyaMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from AyaMusic.utils.stream.autoclear import auto_clean
from AyaMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Aya.pause_stream(chat_id)
        ...
        await Aya.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Aya.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Aya.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Aya.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Aya.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
